...
async function run() {
  try {
    ...
    const brandsCollection = client.db('carDealer').collection('productBrands');
    const categoriesCollection = client.db('carDealer').collection('productCategories');
    const productsCollection = client.db('carDealer').collection('products');
    const blogsCollection = client.db('carDealer').collection('blogs');
    const bookingCollection = client.db('carDealer').collection('booking');
    const paymentsCollection = client.db('carDealer').collection('payments');
...
app.listen(port, () => {
  console.log(`CarDelar listening on port ${port}`)
})